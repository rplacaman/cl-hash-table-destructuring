...
(defmacro with-hash-table-values-fn ((&whole entries entry &rest more-entries) (hash-table-form keyfn) &body body)
...
(defmacro with-hash-table-values ((&whole entries entry &rest more-entries) hash-table-form &body body)
  `(with-hash-table-values-fn ,entries (,hash-table-form *keyfn*)
     ,@body))


(defmacro with-hash-table-items-fn ((&whole entries entry &rest more-entries) (hash-table-form keyfn) &body body)
  (let* ((hash-table-name (gensym (string :hash-table)))
         (keyfn-name (gensym (string :keyfn)))
...
(defmacro with-hash-table-items ((&whole entries entry &rest more-entries) hash-table-form &body body)
